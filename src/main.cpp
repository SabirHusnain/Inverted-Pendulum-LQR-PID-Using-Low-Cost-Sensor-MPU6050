...
/* First of all calibrate MPU6050,
For calibration put it on the flat surface with components
on the top side, hope undertood my point :). Then uncomment
the __calibrate_Sensor and comment all other directive.
After than run the code and wat for sometime, this may take
a while, from the serial monitor after finishing the calibration
process note the gains above the line hacing word done (in the -
serial monitor you will see done which means calibration is done).

Comment out the __calibrate_Sensor and uncomment __run _Control
Now put those gain values in the setup function and choose
controller e.g. PID or LQR uncomment the respective directive,
put the gains, mention pins or use default ones and choose single
motor or dual motor control mode.

Output directives are just to show the outputs in the serial
These are for the verification of MPU6050 outptus or to check
the sampling time.
*/

#define __run_Control
// #define __calibrate_Sensor
// #define __manual_Control
...
#define __pwm_Frequency 3000           // PWM Channel Frequency --> 4KHz
...
#define pin_Clockwise_B 15      // Motor Pin 1 - Motor 2
#define pin_Anti_Clockwise_B 32 // Motor Pin 2 - Motor 2
...
#if defined(__Output_Readable_Real_Acc) || defined(__Output_Readable_World_Acc)
...
double K[] = {201.6684, 2.0978e3, 6.1356}; // {K_p  K_i  K_d}
PID myMotor[motors];
#endif

#ifdef __lqr_Control
// LQR Gains
double K[] = {62.3656, 9.4256, -12.9099, -20.6123};
double LQR_K_I[] = {1.5, 0.5, 0.5, 0.1};
LQR myMotor[motors];
#endif
...
void loop()
{
  ...
  myMotor[0].evaluatePID(delTime, currTheta, 0.0, false);
  myMotor[0].runPID();
...
#ifdef __Output_Motor_Pulses
  Serial.println((long)pulses[0]);
#endif
}
...
{
  Serial.begin(115200);
  pinMode(pin_Clockwise_A, OUTPUT);
  pinMode(pin_Clockwise_B, OUTPUT);
  pinMode(pin_Anti_Clockwise_A, OUTPUT);
  pinMode(pin_Anti_Clockwise_B, OUTPUT);
  ledcSetup(pwm_Channel_A, __pwm_Frequency, __pwm_Resolution);
  ledcAttachPin(pin_Pwm_A, pwm_Channel_A);
  ledcSetup(pwm_Channel_B, __pwm_Frequency, __pwm_Resolution);
  ledcAttachPin(pin_Pwm_B, pwm_Channel_B);
}
void loop()
{
  motorInputA(__clock_Wise, 4095);
  motorInputB(__clock_Wise, 4095);
